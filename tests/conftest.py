...
def client(session):  # Usa a fixture de session que possui o SQLite em memoria
    def get_session_override():
        # Nós criamos a função pois o Depends() pede extritamente uma function
        return session

    with TestClient(app) as client:
        app.dependency_overrides[get_session] = get_session_override  # Sobrescreve para a session na memoria
        yield client
